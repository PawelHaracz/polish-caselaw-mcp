...
describe('getAbout', () => {
  it('reports SAOS as the live source and is not legal advice', () => {
    const out = getAbout();
    expect(out.name).toBe('Polish Case Law MCP');
    expect(out.data_sources[0].name).toContain('SAOS');
    expect(out.freshness.mode).toBe('live');
    expect(out.disclaimer.toLowerCase()).toContain('not legal advice');
  });
});
